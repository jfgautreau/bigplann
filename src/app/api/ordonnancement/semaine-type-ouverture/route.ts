...
export async function POST(req: NextRequest) {
  ...
  if (!quart_code || !ligne_id || jour_semaine === undefined || jour_semaine < 0 || jour_semaine > 6) {
  ...
  const { error } = await supabase
    ...
    .upsert({ quart_code, ligne_id, jour_semaine, ouverte: value }, { onConflict: "quart_code,ligne_id,jour_semaine" });
